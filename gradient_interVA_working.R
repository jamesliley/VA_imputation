#Allow change in probbase for interVA model by epsilon
...
##' 
##' 
##' #Custom letter probabilities and select probbase2[2,17] to change by +0.1
##' 
...
##' 0.01, 0.005, 0, 0, 0.2, 0, 0.00001, 0, 0)), probbase_element=list(2,17), epsilon = 0.1)
##' 
##' 
##' to get causes of death with group code for further usage
##' sample.output2 <- InterVA2(SampleInput, HIV = "h", Malaria = "l", write=FALSE,
##' replicates = FALSE, groupcode = TRUE, probBase=probbase2, letterProb = NULL,probbase_element=list(2,17), epsilon = 0.1)
##' 
##' 
##' Using default probbase and letter probabilities
##' sample.output3 <- InterVA2(SampleInput, HIV = "h", Malaria = "l", write=FALSE,
##' replicates = FALSE, groupcode = TRUE, probBase=NULL, letterProb = NULL,probbase_element=list(2,17), epsilon = 0.1)
                    ...
                    ...)
{
  ...
  #NEW SECTION 2- changing single element of probbase by epsilon
...
##' 
##' 
##' select probbase2[23,55] to change by +0.1
...
##' "C+", "C", "C-", "D+", "D", "D-", "E", "N", ""),
##' value = c(1, 1, 0.7, 0.2, 0.9, 1, 0.02, 0.4,
##' 0.01, 0.005, 0, 0, 0.2, 0, 0.00001, 0, 0)), probbase_element = list(23,55), epsilon = 0.1)
##' 
##' 
...
##' head(out$Imputed)
##' 
##' select probbase2[11,17] to change by +0.1
##' 
##' out=recoverAnswers(RandomVA1,block=11:20,method="OpenVA",
##' data.type = "WHO2012",model = "InterVA",
##' version = "4.03", HIV = "h", Malaria = "h",probBase = probbase2, letterProb = data.frame(
...
##' "C+", "C", "C-", "D+", "D", "D-", "E", "N", ""),
##' value = c(1, 1, 0.7, 0.2, 0.9, 1, 0.02, 0.4,
##' 0.01, 0.005, 0, 0, 0.2, 0, 0.00001, 0, 0)), probbase_element =list(11,17) , epsilon = 0.1)
##' head(out$Original)
##' head(out$Imputed)
...
gradient_probbase<- function(VA, blocks, epsilon, method = "OpenVA", probbase_element, probBase=NULL, letterProb=NULL,
  ...
  if(model == "InterVA"){
    accuracy_new <- evaluateProbbase(VA=VA,
                                     ...
                                     HIV = HIV,
                                     Malaria = Malaria, 
                                     probbase_element = probbase_element, 
                                     epsilon = epsilon)
    }
...
probbase2[,20:50]<-"A+"
out<- gradient_probbase(RandomVA1[1:100,],block=list(18:20, 21:24),method="OpenVA",data.type = "WHO2012",
model = "InterVA",probBase=probbase2, probbase_element = list(10,25) , epsilon = -0.7)

accuracy_original<- evaluateProbbase(RandomVA1[1:100,],block=list(18:20, 21:24,50:55),method="OpenVA",data.type = "WHO2012",
                                     model = "InterVA",probBase=probbase2)

accuracy_new <- evaluateProbbase(RandomVA1[1:100,],block=list(18:20, 21:24,50:55),method="OpenVA",data.type = "WHO2012",
                                 model = "InterVA",probBase=probbase2, probbase_element = list(10,25) , epsilon = -0.7)
#probbase[10,14]="I"=1
#Need to check works for interVA grad as getting lots of zeros
#Once fixed can update all code to github




#Still works for InSilico
## Impute blocks of questions 18-20 and 21-24, use probbase condprobnum and change element (1,1) of probbase by 0.1
#data("condprobnum")
#data("RandomVA1")
#out<- gradient_probbase(RandomVA1[1:100,],block=list(18:20, 21:24),method="OpenVA",data.type = "WHO2012",
...
out1=recoverAnswers3(RandomVA1[1:100,],block=11:20,method="OpenVA",
      data.type = "WHO2012",model = "InterVA",
      version = "4.03", HIV = "h", Malaria = "h",probBase = probbase2, letterProb = data.frame(
        grade = c("I", "A+", "A", "A-", "B+", "B", "B-", "B -", 
                  "C+", "C", "C-", "D+", "D", "D-", "E", "N", ""),
...
#select probbase2[11,17] to change by +0.1
out2=recoverAnswers3(RandomVA1[1:100,],block=11:20,method="OpenVA",
      data.type = "WHO2012",model = "InterVA",
      version = "4.03", HIV = "h", Malaria = "h",probBase = probbase2, letterProb = data.frame(
        grade = c("I", "A+", "A", "A-", "B+", "B", "B-", "B -", 
                  "C+", "C", "C-", "D+", "D", "D-", "E", "N", ""),
        value = c(1, 1, 0.7, 0.2, 0.9, 1, 0.02, 0.4,
                  0.01, 0.005, 0, 0, 0.2, 0, 0.00001, 0, 0)), probbase_element =list(10,14) , epsilon = 0.45)
